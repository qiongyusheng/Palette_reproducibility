...
# B2RNA
B2RNA= denoised_data[batch == 1.,:model.vae.config.dim_input_arr[0]]


if isinstance(B2RNA, np.ndarray):
    latent_df = pd.DataFrame(B2RNA)
else:
    
    latent_df = B2RNA


latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B3RNA.csv', index=False)
...
latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B3ADT.csv', index=False)
...
latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B4ATAC.csv', index=False)

# B3ADT
B3ADT= denoised_data[batch == 2.,model.vae.config.dim_input_arr[0]:(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1])]


if isinstance(B3ADT, np.ndarray):
    latent_df = pd.DataFrame(B3ADT)
else:
    
    latent_df = B3ADT


latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B4ADT.csv', index=False)
...
latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B5ATAC.csv', index=False)
...
latent_df.to_csv('./imputation/TEA_s1/scVAEIT/B5RNA.csv', index=False)

peak = pd.DataFrame(peak_name1)
peak.to_csv('./imputation/TEA_s1/scVAEIT/peak.csv', index=False)

gene = pd.DataFrame(gene_name1)
gene.to_csv('./imputation/TEA_s1/scVAEIT/gene.csv', index=False)
