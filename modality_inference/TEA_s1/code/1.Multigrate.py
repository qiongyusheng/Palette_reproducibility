...
# B3 rna
B3_rna = infer_res1[6194:12575,:]
if isinstance(B3_rna, np.ndarray):
    latent_df = pd.DataFrame(B3_rna)
else:
    # 如果 latent_data 已经是 DataFrame，则直接使用
    latent_df = B3_rna
# 保存为 CSV 文件
latent_df.to_csv('./imputation/TEA_s1/Multigrate/B3_rna.csv', index=False)
...
if isinstance(B3_adt, np.ndarray):
    ...
    latent_df = B3_adt
# 保存为 CSV 文件
latent_df.to_csv('./imputation/TEA_s1/Multigrate/B3_adt.csv', index=False)
...
io.mmwrite("./imputation/TEA_s1/Multigrate/B4_atac.mtx", csr_matrix)

# B4 adt
B4_adt = infer_res3[12575:18987,:]
if isinstance(B4_adt, np.ndarray):
    latent_df = pd.DataFrame(B4_adt)
else:
    # 如果 latent_data 已经是 DataFrame，则直接使用
    latent_df = B4_adt
# 保存为 CSV 文件
latent_df.to_csv('./imputation/TEA_s1/Multigrate/B4_adt.csv', index=False)

# B5 atac
B5_atac = infer_res2[18987:25517,:]
csr_matrix = sparse.csr_matrix(B5_atac)
io.mmwrite("./imputation/TEA_s1/Multigrate/B5_atac.mtx", csr_matrix)
...
if isinstance(B5_rna, np.ndarray):
    ...
    latent_df = B5_rna
# 保存为 CSV 文件
latent_df.to_csv('./imputation/TEA_s1/Multigrate/B5_rna.csv', index=False)