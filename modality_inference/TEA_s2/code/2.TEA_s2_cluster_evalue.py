...
B2_rna_df = cal_metrics(
    path ='./imputation/TEA_s2/',
    data_path = ['Palette/B2_rna.csv','MIDAS/B2_rna.csv',
                 'Multigrate/B2_rna.csv','scVAEIT/B2RNA.csv'],              # list[str]：每个方法的数据文件相对 path 的路径
    method_name = ['Palette','MIDAS','Multigrate','scVAEIT'],            # list[str]：方法名（与 data_path 对齐）
    meta_path = './PBMC_TEA/B2/meta.rds',              # RDS/CSV：包含真实标签
...
rna_df = pd.concat([B2_rna_df,B5_rna_df], axis=0)
rna_df['Task'] = 'RNA'

B2_adt_df = cal_metrics(
    path ='./imputation/TEA_s2/',
    data_path = ['Palette/B2_adt.csv','MIDAS/B2_adt.csv',
                 'Multigrate/B2_adt.csv','scVAEIT/B2ADT.csv'],              # list[str]：每个方法的数据文件相对 path 的路径
    method_name = ['Palette','MIDAS','Multigrate','scVAEIT'],            # list[str]：方法名（与 data_path 对齐）
    meta_path = './PBMC_TEA/B2/meta.rds',              # RDS/CSV：包含真实标签
    label_key = 'celltype.l2',              # 真实标签列名
...
adt_df = pd.concat([B2_adt_df,B3_adt_df], axis=0)
adt_df['Task'] = 'ADT'
...
df.to_csv('./imputation/TEA_s2/metrics_cluster.csv', index=False)
