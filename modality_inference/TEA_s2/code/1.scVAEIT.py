...
# B1RNA
B1RNA= denoised_data[batch == 0.,:model.vae.config.dim_input_arr[0]]


if isinstance(B1RNA, np.ndarray):
    latent_df = pd.DataFrame(B1RNA)
else:
    
    latent_df = B1RNA


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B2RNA.csv', index=False)

# B1ADT
B1ADT= denoised_data[batch == 0.,model.vae.config.dim_input_arr[0]:(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1])]


if isinstance(B1ADT, np.ndarray):
    latent_df = pd.DataFrame(B1ADT)
else:
    
    latent_df = B1ADT


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B2ADT.csv', index=False)

# B2ADT
B2ADT= denoised_data[batch == 1.,model.vae.config.dim_input_arr[0]:(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1])]


if isinstance(B2ADT, np.ndarray):
    latent_df = pd.DataFrame(B2ADT)
else:
    
    latent_df = B2ADT


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B3ADT.csv', index=False)

# B3ATAC
B3ATAC= denoised_data[batch == 2.,(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]):]


if isinstance(B3ATAC, np.ndarray):
    latent_df = pd.DataFrame(B3ATAC)
else:
    
    latent_df = B3ATAC


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B4ATAC.csv', index=False)

# B4ATAC
B4ATAC= denoised_data[batch == 3.,(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]):]


if isinstance(B4ATAC, np.ndarray):
    latent_df = pd.DataFrame(B4ATAC)
else:
    
    latent_df = B4ATAC


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B5ATAC.csv', index=False)

# B4RNA
B4RNA= denoised_data[batch == 3.,:model.vae.config.dim_input_arr[0]]


if isinstance(B4RNA, np.ndarray):
    latent_df = pd.DataFrame(B4RNA)
else:
    
    latent_df = B4RNA


latent_df.to_csv('./imputation/TEA_s2/scVAEIT/B5RNA.csv', index=False)


peak = pd.DataFrame(peak_name1)
peak.to_csv('./imputation/TEA_s2/scVAEIT/peak.csv', index=False)

gene = pd.DataFrame(gene_name1)
gene.to_csv('./imputation/TEA_s2/scVAEIT/gene.csv', index=False)
