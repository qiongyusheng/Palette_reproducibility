...
# cite 1 adt
cite1adt = denoised_data[batch == 0.,model.vae.config.dim_input_arr[0]:(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1])]


if isinstance(cite1adt, np.ndarray):
    latent_df = pd.DataFrame(cite1adt)
else:
    
    latent_df = cite1adt


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/cite1adt.csv', index=False)

del cite1adt
del latent_df

# cite 1 rna
cite1rna = denoised_data[batch == 1.,0:model.vae.config.dim_input_arr[0]]


if isinstance(cite1rna, np.ndarray):
    latent_df = pd.DataFrame(cite1rna)
else:
    
    latent_df = cite1rna


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/cite1rna.csv', index=False)

del cite1rna
del latent_df

# cite 2 adt
cite2adt = denoised_data[batch == 2.,model.vae.config.dim_input_arr[0]:(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1])]


if isinstance(cite2adt, np.ndarray):
    latent_df = pd.DataFrame(cite2adt)
else:
    
    latent_df = cite2adt


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/cite2adt.csv', index=False)

del cite2adt
del latent_df

# cite 2 rna
cite2rna = denoised_data[batch == 3.,0:model.vae.config.dim_input_arr[0]]


if isinstance(cite2rna, np.ndarray):
    latent_df = pd.DataFrame(cite2rna)
else:
    
    latent_df = cite2rna


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/cite2rna.csv', index=False)

del cite2rna
del latent_df

# mul 2 atac
mul2atac = denoised_data[batch == 6.,(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]):(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]+model.vae.config.dim_input_arr[2])]


if isinstance(mul2atac, np.ndarray):
    latent_df = pd.DataFrame(mul2atac)
else:
    
    latent_df = mul2atac


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/mul2atac.csv', index=False)

del mul2atac
del latent_df

# mul 2 rna
mul2rna = denoised_data[batch == 7.,0:model.vae.config.dim_input_arr[0]]


if isinstance(mul2rna, np.ndarray):
    latent_df = pd.DataFrame(mul2rna)
else:
    
    latent_df = mul2rna


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/mul2rna.csv', index=False)

del mul2rna
del latent_df

# mul 3 atac
mul3atac = denoised_data[batch == 8.,(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]):(model.vae.config.dim_input_arr[0]+model.vae.config.dim_input_arr[1]+model.vae.config.dim_input_arr[2])]


if isinstance(mul3atac, np.ndarray):
    latent_df = pd.DataFrame(mul3atac)
else:
    
    latent_df = mul3atac


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/mul3atac.csv', index=False)

del mul3atac
del latent_df

# mul 3 rna
mul3rna = denoised_data[batch == 9.,0:model.vae.config.dim_input_arr[0]]


if isinstance(mul3rna, np.ndarray):
    latent_df = pd.DataFrame(mul3rna)
else:
    
    latent_df = mul3rna


latent_df.to_csv('./imputation/BMMC_s/scVAEIT/mul3rna.csv', index=False)
...
peak.to_csv('./imputation/BMMC_s/scVAEIT/peak.csv', index=False)

gene = pd.DataFrame(gene_name1)
gene.to_csv('./imputation/BMMC_s/scVAEIT/gene.csv', index=False)