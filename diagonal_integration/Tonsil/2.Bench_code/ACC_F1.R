...
meta_codex <- dplyr::filter(meta,modality == "CODEX")
...
pred_label_df1$ground_truth <- meta_rna$celltype

for(i in 1:length(method)){
    if(method[i] != 'Seurat'){
    pred_tmp <- Trans.Knowledge(res[[i]][12978:nrow(meta),],
        res[[i]][1:12977,],
...
pred_label_df2$ground_truth <- meta_codex$celltype

for(i in 1:length(method)){
    if(method[i] != 'Seurat'){
        pred_tmp <- Trans.Knowledge(res[[i]][1:12977,],res[[i]][12978:nrow(meta),],
...
rna <- as.matrix(readRDS('./RNA/co_all.rds'))
codex <- as.matrix(readRDS('./CODEX/co_all.rds'))
...
rownames(meta) <- c(colnames(rna),colnames(codex))
seurat_res <- run_Seurat(list(rna,codex),
                         meta = meta,
                         is.normalize = FALSE,
                         nfeatures = 51,
                         reduction = 'cca',
...
saveRDS(out1,'./ACC_macro_f1_CODEX2RNA.rds')
saveRDS(out2,'./ACC_macro_f1_RNA2CODEX.rds')
...
saveRDS(pred_label_df1,'./pred_label_all_methods_RNA.rds')
saveRDS(pred_label_df2,'./pred_label_all_methods_CODEX.rds')