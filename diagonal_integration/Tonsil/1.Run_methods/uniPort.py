...
adt = np.array(pyreadr.read_r('./CODEX/codex.rds')[None]).T
...
adt_co = np.array(pyreadr.read_r('./CODEX/co_all.rds')[None]).T
...
sc.pp.highly_variable_genes(adata_cm, n_top_genes=51, inplace=False, subset=True)
...
sc.pp.highly_variable_genes(adt_adata, n_top_genes=46, inplace=False, subset=True)
