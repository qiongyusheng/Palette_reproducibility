...
setwd('/home/server/sqy/data/mosaic/input/MERFISH_ATAC')

atac <- readRDS('./ATAC/atac.rds')
atac_co <- readRDS('./ATAC/co200_RF.rds')

rna_co <- readRDS('./MERFISH/co200_RF.rds')
...
x_input = as.matrix(fread('./MERFISH/bdsc30_200RF.csv',data.table = F))[1:30,]

# y_input = atac_n
y_input <- as.matrix(t(svd_res@cell.embeddings[,1:30]))
z0_input = as.matrix(fread('./ATAC/bdsc30_200RF.csv',data.table = F))[1:30,]
