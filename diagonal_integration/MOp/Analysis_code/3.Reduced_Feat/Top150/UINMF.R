...
rna <- readRDS('./MERFISH/co150_RF.rds')
atac <- readRDS('./ATAC/atac.rds')
atac_co <- readRDS('./ATAC/co150_RF.rds')

lig <- createLiger(list(rna = as(as.matrix(rna),'dgCMatrix'), atac = rbind(as(as.matrix(atac_co),'dgCMatrix'),atac)))
lig <- normalize(lig) %>%
  selectGenes(useDatasets = "rna", thresh = 0) %>%
  scaleNotCenter()

unshareNormed <- normalize(atac)
