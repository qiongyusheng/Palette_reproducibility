...
top150 = pyreadr.read_r('./MERFISH_ATAC/RF150.rds')[None]
feat = top150['V1'].tolist()
del top150
...
new_names = {}
for var in atac_adata.var_names:
    if var not in feat:
        new_names[var] = 'atac_' + var

atac_adata.var_names = [new_names.get(x, x) for x in atac_adata.var_names]
del new_names
...
new_names = {}
for var in rna_adata.var_names:
    if var not in feat:
        new_names[var] = 'rna_' + var

rna_adata.var_names = [new_names.get(x, x) for x in rna_adata.var_names]
del new_names
...
sc.pp.highly_variable_genes(adata_cm, n_top_genes=150, inplace=False, subset=True)
...
latent_df.to_csv('./Reduced_feat/RF150/uniport.csv', 
                 index=False)
