...
atac_co <- readRDS('./ATAC/co150_RF.rds')

rna_co <- readRDS('./MERFISH/co150_RF.rds')
...
x_input = as.matrix(fread('./MERFISH/bdsc30_150RF.csv',data.table = F))

# y_input = atac_n
y_input <- as.matrix(t(svd_res@cell.embeddings[,1:30]))
z0_input = as.matrix(fread('./ATAC/bdsc30_150RF.csv',data.table = F))
