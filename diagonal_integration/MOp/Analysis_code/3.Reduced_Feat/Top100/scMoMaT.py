...
for batch in range(n_batches):
    if batch in [0,1]:
        rds_data = pyreadr.read_r(os.path.join(dir, path[batch] + "/co100_RF.rds"))
...
rds_data = pyreadr.read_r(os.path.join(dir, path[0] + "/co100_RF.rds"))
...
latent_df.to_csv('./Reduced_feat/RF100/scMoMaT.csv', index=False)