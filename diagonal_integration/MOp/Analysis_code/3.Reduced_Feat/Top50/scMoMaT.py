...
for batch in range(n_batches):
    if batch in [0,1]:
        rds_data = pyreadr.read_r(os.path.join(dir, path[batch] + "/co50_RF.rds"))
...
rds_data = pyreadr.read_r(os.path.join(dir, path[0] + "/co50_RF.rds"))
...
latent_df.to_csv('./Reduced_feat/RF50/scMoMaT.csv', index=False)