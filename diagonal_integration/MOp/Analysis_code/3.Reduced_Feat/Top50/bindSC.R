...
atac_co <- readRDS('./ATAC/co50_RF.rds')

rna_co <- readRDS('./MERFISH/co50_RF.rds')
...
y_obj <- RunPCA(y_obj, features = VariableFeatures(object = y_obj),npcs = 30)
...
## now gather all the actual inputs
x_input = as.matrix(fread('./MERFISH/bdsc30_50RF.csv',data.table = F))

y_input <- as.matrix(t(svd_res@cell.embeddings[1:30]))
z0_input = as.matrix(fread('./ATAC/bdsc30_50RF.csv',data.table = F))
