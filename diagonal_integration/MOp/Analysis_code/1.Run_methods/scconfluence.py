...
atac = np.array(pyreadr.read_r('./input/MERFISH_ATAC/ATAC/LSI.rds')[None])
meta_atac = pyreadr.read_r('./input/MERFISH_ATAC/ATAC/meta.rds')[None]
atac_adata = ad.AnnData(
    atac, dtype=np.float32,obs=meta_atac
)

merf_co = np.array(pyreadr.read_r('./MERFISH_ATAC/MERFISH/co.rds')[None]).T
atac_co = np.array(pyreadr.read_r('./MERFISH_ATAC/ATAC/co.rds')[None]).T
...
mdata['rna'].obs_names = [f"rna{i+1}" for i in range(mdata['rna'].n_obs)]
mdata['atac'].obs_names = [f"atac{i+1}" for i in range(mdata['atac'].n_obs)]
