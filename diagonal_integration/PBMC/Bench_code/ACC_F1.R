...
setwd('./')
method = c("Palette_unnorm","stab","Multigrate","scMoMaT",
           "midas",'scVAEIT',"bindsc",'uniport',"UINMF",
            "fastMNN","Seurat","Harmony","maxfuse",'scCon')
...
meta_cytof <- dplyr::filter(meta,modality == "CyTOF")
...
for(i in 1:length(method)){
    ...
    if(method[i] != 'Seurat'){
      pred_tmp <- Trans.Knowledge(res[[i]][76070:nrow(meta),],
        res[[i]][1:76069,],
                      data.frame(meta_cytof[,3]))
    
    pred_label_df1[,method[i]] <- pred_tmp[,1]
    score_tmp <- evaluate(meta_rna[,3],pred_tmp[,1])
...
pred_label_df2 <- as.data.frame(matrix(0,nrow = nrow(meta_cytof),ncol = length(method)+1))
colnames(pred_label_df2) <- c('ground_truth',method)
rownames(pred_label_df2) <- rownames(meta_cytof)
pred_label_df2$ground_truth <- meta_cytof$label

for(i in 1:length(method)){
    if(method[i] != 'Seurat'){
      pred_tmp <- Trans.Knowledge(res[[i]][1:76069,],res[[i]][76070:nrow(meta),],
                      data.frame(meta_rna[,3]))
    
    pred_label_df2[,method[i]] <- pred_tmp[,1]
    score_tmp <- evaluate(meta_cytof[,3],pred_tmp[,1])
                      ...
                      out.npcs = 30)
{
  ...
  gc()
  p1 = TransferData(anchorset = anchors,batch_list[[1]][['CellType']][,1])
   anchors = FindTransferAnchors(
  ...
  p2 = TransferData(anchorset = anchors,batch_list[[2]][['CellType']][,1])
  t2 = Sys.time()
  print(t2-t1)
...
seurat_res <- run_Seurat(list(rna,cytof),
                         meta = meta,
                         nfeatures = 35,
                         reduction = 'cca',
                         out.npcs = 20)

score_tmp <- evaluate(meta_rna[,1],seurat_res[[2]][,1])
out1['Seurat','ACC'] <- score_tmp[[1]]
out1['Seurat','macro_F1'] <- score_tmp[[1]]

score_tmp <- evaluate(meta_cytof[,1],seurat_res[[1]][,1])
out2['Seurat','ACC'] <- score_tmp[[1]]
out2['Seurat','macro_F1'] <- score_tmp[[1]]

rownames(out1) <- c("Palette","StabMap","Multigrate","scMoMaT",
                    "midas",'scVAEIT',"bindSC",'uniPort',"UINMF",
                    "fastMNN","Seurat","Harmony",'MaxFuse','scConfluence')
...
saveRDS(out1,'./ACC_macro_f1_CyTOF2RNA.rds')
saveRDS(out2,'./ACC_macro_f1_RNA2CyTOF.rds')
