...
tmp = sp.vstack([atacs[0], atacs[2]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * atacs[0].shape[0] + ["batch2"] * atacs[2].shape[0]
...
latent_df.to_csv('./Benchmarking/Unsupervised/TEA_s1/output/scVAEIT_v3.csv', index=False)