...
parser$add_argument("--task", type = "character", default = "BMMC_s35")
...
config <- parseTOML("./midas/configs/data.toml")[[gsub("_vd.*|_vt.*|_transfer$|_ref_.*$", "", o$task)]]
...
plus <- c(1:10)
subset_names <- paste0(subset_names,plus)
...
# subset_names <- c('s1d1','s1d2','s1d3','s1d1','s1d2','s1d3')
...
write.csv(c,"./Benchmarking/Unsupervised/BMMC_s2/output/midas_35.csv",row.names=FALSE)
