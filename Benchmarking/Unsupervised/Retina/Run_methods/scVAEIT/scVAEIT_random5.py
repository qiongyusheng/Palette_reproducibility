...
tmp = sp.vstack([atacs[1], atacs[3], atacs[5], atacs[6], atacs[7]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * atacs[1].shape[0] + ["batch2"] * atacs[3].shape[0] + ["batch3"] * atacs[5].shape[0] + ["batch4"] * atacs[6].shape[0] + ["batch5"] * atacs[7].shape[0]
...
latent_df.to_csv('./Benchmarking/Unsupervised/Retina/output/scVAEIT_s4.csv', index=False)
