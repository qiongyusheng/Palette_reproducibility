...
tmp = sp.vstack([atacs[0], atacs[2], atacs[3], atacs[4], atacs[7]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * atacs[0].shape[0] + ["batch2"] * atacs[2].shape[0] + ["batch3"] * atacs[3].shape[0] + ["batch4"] * atacs[4].shape[0] + ["batch5"] * atacs[7].shape[0]
...
latent_df.to_csv('./Benchmarking/Unsupervised/Retina/output/scVAEIT_s3.csv', index=False)
