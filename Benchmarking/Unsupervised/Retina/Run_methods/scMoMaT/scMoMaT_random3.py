...
import scipy.sparse as sp
tmp = sp.vstack([counts_atacs[0], counts_atacs[1], counts_atacs[5], counts_atacs[6], counts_atacs[7]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * counts_atacs[0].shape[0] + ["batch2"] * counts_atacs[1].shape[0] + ["batch3"] * counts_atacs[5].shape[0] + ["batch4"] * counts_atacs[6].shape[0] + ["batch5"] * counts_atacs[7].shape[0]
...
latent_df.to_csv('./Benchmarking/Unsupervised/Retina/output/scMoMaT.csv', index=False)