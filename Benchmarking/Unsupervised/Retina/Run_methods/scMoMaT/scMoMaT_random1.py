...
import scipy.sparse as sp
tmp = sp.vstack([counts_atacs[0], counts_atacs[1], counts_atacs[3], counts_atacs[5], counts_atacs[6]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * counts_atacs[0].shape[0] + ["batch2"] * counts_atacs[1].shape[0] + ["batch3"] * counts_atacs[3].shape[0] + ["batch4"] * counts_atacs[5].shape[0] + ["batch5"] * counts_atacs[6].shape[0]
...
counts_atacs[3] = counts_atac3
del adata_hvg3
del counts_atac3
del hvg3_d

adata_hvg6 = adata_hvg[adata.obs['batch'] == 'batch5',:]
...
latent_df.to_csv('./Benchmarking/Unsupervised/Retina/output/scMoMaT_s1.csv', index=False)