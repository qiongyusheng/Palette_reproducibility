...
import scipy.sparse as sp
tmp = sp.vstack([counts_atacs[0], counts_atacs[2], counts_atacs[3], counts_atacs[4], counts_atacs[7]])
import anndata as ad
import scanpy as sc
adata = ad.AnnData(tmp,dtype=np.float32)
adata.obs['batch'] = ["batch1"] * counts_atacs[0].shape[0] + ["batch2"] * counts_atacs[2].shape[0] + ["batch3"] * counts_atacs[3].shape[0] + ["batch4"] * counts_atacs[4].shape[0] + ["batch5"] * counts_atacs[7].shape[0]
...
latent_df.to_csv('./Benchmarking/Unsupervised/Retina/output/scMoMaT_s3.csv', index=False)