...
task <- "Retina_s4"
config <- parseTOML("./midas/configs/data.toml")[[gsub("_vd.*|_vt.*", "", task)]]
combs <- config$combs
...
merge_counts <- function(mod) {
  ...
  for (dataset_id in seq_along(input_dirs)) {
    ...
    # gn <- pj(input_dirs[dataset_id], paste0('gene.csv'))
    # bcd <- pj(input_dirs[dataset_id], paste0('meta.rds'))  
    if (file.exists(fp)) {
      prt("Loading ", fp, " ...")
      # gene <- fread(gn,data.table = F)
      # meta <- readRDS(bcd)
      # sc <- readMM(fp)
      # rownames(sc) <- gene[,1]
      # colnames(sc) <- rownames(meta)
      sc <- readRDS(fp)
      cell_num <- dim(sc)[2]
...
merge_frags <- function() {
  ...
  for (dataset_id in seq_along(input_dirs)) {
    ...
    comb_ratio <- comb_ratios[[dataset_id]]
    fp <- pj(input_dirs[dataset_id], paste0(config[[mod]][dataset_id]))
    pn <- pj(input_dirs[dataset_id], paste0('peak.csv'))
    bcd <- pj(input_dirs[dataset_id], paste0('bcd.csv'))  
    if (file.exists(fp)) {
      prt("Loading ", fp, " ...")
      peak <- fread(pn,data.table = F)
      cells <- fread(bcd,data.table = F)
      sc <- readMM(fp)
      rownames(sc) <- peak[,1]
      colnames(sc) <- cells[,1]
      # sc <- readRDS(fp)
  ...
  feat_merged <- Reduce(union, feat_list) # Signac::reduce(do.call("c", unname(feat_list)))
