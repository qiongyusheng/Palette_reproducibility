...
latent_data <- t(as.matrix(fread('./Benchmarking/Reference-based_integration/output/Palette_Reference/Palette_unsupervised.csv',data.table = F)))
...
latent_data <- t(as.matrix(fread('./Benchmarking/Reference-based_integration/output/Palette_Reference/Palette_unsupervised_30.csv',
                                 data.table = F)))
colnames(latent_data) <- rownames(meta)
...
write.csv(as.matrix(t(result_harmony[,rownames(meta)])),
          paste0(out_dir,'Palette_harmony.csv'),
          row.names = FALSE)
write.csv(as.matrix(t(result_fastMNN[,rownames(meta)])),
          paste0(out_dir,'Palette_fastMNN.csv'),
          row.names = FALSE)
write.csv(as.matrix(t(result_Seurat[,rownames(meta)])),
          paste0(out_dir,'Palette_Seurat.csv'),
          row.names = FALSE)

saveRDS(meta,paste0(out_dir,'meta.rds'))
saveRDS(meta_q,paste0(out_dir,'meta_query.rds'))
saveRDS(meta_ref,paste0(out_dir,'meta_ref.rds'))
saveRDS(score,paste0(out_dir,'score.rds'))
