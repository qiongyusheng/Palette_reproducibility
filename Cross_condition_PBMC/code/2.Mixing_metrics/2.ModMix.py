...
out_dir = './Cross_condition_PBMC/output/'
meta_dir = './Cross_condition_PBMC/output/Embedding/'
...
dataset = np.random.rand(17829, 50)
meta = meta[None]
adata = anndata.AnnData(X=dataset, obs=meta)

data_dir = './Cross_condition_PBMC/output/Embedding/'
...
r_df = pyreadr.read_r('./Cross_condition_PBMC/output/Embedding/' + "lisi_score.rds")[None]
...
metric_df.loc['Metric Type'] = ['Modality mixing','Modality mixing','Modality mixing',
                               'Aggregate score']
...
latent_df.to_csv('./Cross_condition_PBMC/output/Embedding/' + 'modality_mixing.csv', index=True)
