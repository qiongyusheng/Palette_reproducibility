...
for batch in range(n_batches):
    ...
    if batch in atac_t:
        ...
        obs['modality'] = np.repeat('asap', counts_atac.shape[0])
    ...
    if batch in adt_t:
        ...
        if batch in [0,1]:
            obs['modality'] = np.repeat('asap', counts_protein.shape[0])
        else:
        ...
        adt_ad.X = adt_ad.X.astype(np.float64)
...
latent_df.to_csv('./Cross_condition_PBMC/output/Embedding/Multigrate.csv', index=False)
