...
def convert_tensors_to_cuda(x):
    """
    Recursively converts tensors to CUDA if GPU is available
    """
    ...
    y = {}
    for kw, arg in x.items():
        y[kw] = arg.to(device) if th.is_tensor(arg) else convert_tensors_to_cuda(arg)
    return y
...
def gen_data_config(task, reference=''):
    if "continual" in task:
        ...
        data_config = load_toml("/home/server/sqy/code/midas/configs/data.toml")[re.sub("_continual", "", task)]
        data_config_ref = load_toml("/home/server/sqy/code/midas/configs/data.toml")[reference]
        ...
        data_config = load_toml("/home/server/sqy/code/midas/configs/data.toml")[cfg_task]
    return data_config
