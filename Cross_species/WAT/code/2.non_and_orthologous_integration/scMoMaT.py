...
for batch in range(n_batches):
    ...
    if batch in range(22):
        rds_data = pyreadr.read_r(os.path.join(dir, path[batch] + "/non.rds"))
        counts_human = np.array(rds_data[None]).T
        counts_human = utils.preprocess(counts_human, modality = "RNA", log = False)
        # counts_rna = pyreadr.read_r(os.path.join(dir, 'rna' + str(batch + 1) + ".rds")).toarray().T
    else:
        counts_human = None
        
    if batch in range(22, 22 + 14):
        rds_data = pyreadr.read_r(os.path.join(dir, path[batch] + "/non.rds"))
        counts_mouse = np.array(rds_data[None]).T
        counts_mouse = utils.preprocess(counts_mouse, modality = "RNA", log = False)
        # counts_rna = pyreadr.read_r(os.path.join(dir, 'rna' + str(batch + 1) + ".rds")).toarray().T
    else:
        counts_mouse = None
        
    counts_rnas.append(counts_rna)
    counts_humans.append(counts_human)
    counts_mouses.append(counts_mouse)

counts = {"rna":counts_rnas, "human": counts_humans, 'mouse':counts_mouses}
...
rds_data = pyreadr.read_r(os.path.join(dir, path[0] + "/non.rds"))
non_hs = rds_data[None].index
rds_data = pyreadr.read_r(os.path.join(dir, path[35] + "/non.rds"))
non_ms = rds_data[None].index

feats_name = {"rna": genes, "human": non_hs, 'mouse':non_ms}
...
latent_df.to_csv('./scMoMaT_non.csv', index=False)
