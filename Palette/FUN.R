...
Add.Assay <- function(assay, musa.object, check = TRUE, class = "original",name = NULL){
  ...
  if(Reduce("+",modal_logic) == 0 && Reduce("+",sample_logic) == 0){
    ...
    if(check){
      #新assay有modal或sample对应，对于modal，取特征交集，对于sample，取细胞交集
...
DownSample <- function(object,
  ...
  for(i in 1:length(modal)){
    ...
    # names(Cluster_list) <- cluster.name_i
...
Build.Kernel.cluster3 <- function(data.list,
  ...
  if(joint && dim.reduce){
    ...
    subKernel <- list()
    scaler <- c()
    # message('Pairwise SVD reduction and L2 normalization were performed on the dataset.')
...
Find_SubSpace3 <- function(object,
  ...
  # snn.input <- lapply(idx,function(x) object@Assays[[x]]@snn[object@Assays[[x]]@RepresentativeCell[[1]],object@Assays[[x]]@RepresentativeCell[[1]]])
  # names(snn.input) <- names(object@Assays)[idx]
