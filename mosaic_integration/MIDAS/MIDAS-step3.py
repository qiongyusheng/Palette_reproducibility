...
## Task
parser.add_argument('--task', type=str, default='TEA_s2',
    help="Choose a task")
...
def load_data_config():
    ...
    if "continual" in o.task:
        o.continual = True
        o.dim_s_query = len(utils.load_toml("/home/server/sqy/code/midas/configs/data.toml")[re.sub("_continual", "", o.task)]["s_joint"])
        o.dim_s_ref = len(utils.load_toml("/home/server/sqy/code/midas/configs/data.toml")[o.reference]["s_joint"])
    else:
        o.continual = False
    ...
    if o.reference != '' and o.continual == False and o.map_ref == 1:  # map query onto reference for transfer learning
        ...
        data_config_ref = utils.load_toml("/home/server/sqy/code/midas/configs/data.toml")[cfg_task_ref]
...
def load_model_config():
    model_config = utils.load_toml("/home/server/sqy/code/midas/configs/model.toml")["default"]
    if o.model != "default":
        model_config.update(utils.load_toml("/home/server/sqy/code/midas/configs/model.toml")[o.model])
...
def init_model():
    ...
    # Initialize models
    net = models.Net(o)
    discriminator = models.Discriminator(o)
...
# Count number of parameters
    net_param_num = sum([param.numel() for param in net.parameters()])
    disc_param_num = sum([param.numel() for param in discriminator.parameters()])
...
parser = argparse.ArgumentParser()
## Task
parser.add_argument('--task', type=str, default='bench',
    help="Choose a task")
parser.add_argument('--reference', type=str, default='',
    help="Choose a reference task")
parser.add_argument('--experiment', type=str, default='e0',
...
parser.add_argument('--model', type=str, default='default',
    help="Choose a model configuration")
parser.add_argument('--actions', type=str, nargs='+', default=['predict_all_latent_bc'],
    help="Choose actions to run")
parser.add_argument('--method', type=str, default='midas',
    help="Choose an method to benchmark")
